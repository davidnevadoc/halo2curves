...
fn criterion_benchmark<G: CurveExt>(c: &mut Criterion, name: &'static str) {
    // G1Projective
    {
        ...
        c.bench_function(
            &format!("G1 Projective of {} check on curve", name),
            move |b| b.iter(|| black_box(p1).is_on_curve()),
        );
        c.bench_function(
            &format!("G1 Projective of {} check equality", name),
            move |b| b.iter(|| black_box(p1) == black_box(p1)),
        );
        ...
        c.bench_function(
            &format!("G1 Projective of {} mixed addition", name),
            move |b| b.iter(|| black_box(p2).add(&p1_affine)),
        );
        c.bench_function(
            &format!("G1 Projective of {} scalar multiplication", name),
            move |b| b.iter(|| black_box(p1) * black_box(s)),
        );
        c.bench_function(
            &format!("G1 Projective of {} batch to affine n={}", name, N),
            move |b| {
                b.iter(|| {
